...
public class dataAdministrator {
  ...
  dataAdministrator(){
    ...
    this.dataCount = 0;
    this.Tshow = 10.0;
    this.amScale =2.0/(1<<15);  // the arduino code is configured to get accelerations in the range [ -16.0 , 16.0 ] g
    this.wmScale =16000.0*PI/180.0/(1<<15);  // the arduino code is configured to get angular velocities in the range [ -2000.0 , 2000.0 ] degrees/s
    ...
    this.nano = new IPM_Nano33IoT( (byte)16 );
  ...
  private void set_measurementIMU( byte[] b ) {
    ...
    switch( b[0] ){  // information packet ID
      ...
      case 1:  // IPM_MPU6050
        ...
        wm = this.agtm.get_w();
        break;
      case 2:  // IPM_AdafruitIMU9dof
        this.amtgt.set_bytes( b );
        am = this.amtgt.get_a();
        wm = this.amtgt.get_w();
        break;
      case 3: //Nano33IoT
        this.nano.set_bytes( b );
        am=this.nano.get_a();
        wm=this.nano.get_w();
        break;
      default:
